...
export default function Home() {
  ...
  const VIDEO_CONSTRAINTS = { width: { ideal: 2560 }, height: { ideal: 1440 }, frameRate: { ideal: 30 } };
  ...
  const boostVideoQuality = async (pc) => {
    ...
    try {
      ...
      params.encodings[0].maxBitrate = 6_000_000; // ~6 Мбит/с — под 1440p ("2K"). Без TURN на слабой сети WebRTC сам занизит битрейт, чтобы не рвать связь.
  ...
  useEffect(() => {
    ...
    const chan = supabase.channel(`user_calls_${session.user.id}`, { config: { broadcast: { self: false } } })
      ...
      .on('broadcast', { event: 'call-answer' }, async ({ payload }) => {
        ...
        if (pcRef.current.signalingState === 'stable') return; // уже подключились через базу — не применяем повторно
    ...
    const dbChan = supabase.channel(`db_calls_${session.user.id}`)
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'calls' }, ({ new: row }) => {
        ...
        if (row.status === 'accepted' && row.answer && pcRef.current && pcRef.current.signalingState !== 'stable') {
          pcRef.current.setRemoteDescription(new RTCSessionDescription(row.answer)).then(() => {
            clearTimeout(ringTimeoutRef.current);
            setCallState('active');
            if (!callTimerRef.current) callTimerRef.current = setInterval(() => setCallDuration(d => d + 1), 1000);
          }).catch(() => {});
        }
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', display: 'flex', background: '#030712', fontFamily: 'system-ui, sans-serif', color: '#f8fafc', overflow: 'hidden' }}>
      <div style={{ width: (activeChat || isSupportMode) ? '320px' : '100%', display: (activeChat || isSupportMode) ? 'none' : 'flex', flexDirection: 'column', borderRight: `1px solid ${theme.border}`, background: 'rgba(11, 15, 25, 0.85)', backdropFilter: 'blur(16px)', height: '100%' }}>
                ...
                if (c?.type === 'group' || c?.type === 'channel') {
                  return (
                    <div key={i} onClick={() => openChannelOrGroup(c)} style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', background: activeChat === c.id ? 'rgba(255,255,255,0.05)' : 'transparent' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ color: '#fff', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '4px' }}>{c.name} {c.is_verified && <Icons.Verified />}</div>
        ...
        <div style={{ display: 'flex', borderTop: `1px solid ${theme.border}`, background: 'rgba(7, 10, 18, 0.92)', backdropFilter: 'blur(16px)', flexShrink: 0, paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          <button onClick={() => { setActiveTab('chats'); setIsSupportMode(false); }} style={{ flex: 1, padding: '10px 4px 8px', background: 'transparent', border: 'none', color: activeTab === 'chats' ? theme.primary : '#64748b', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
            <span style={{ fontSize: '20px', filter: activeTab === 'chats' ? `drop-shadow(0 0 6px ${theme.glow})` : 'none' }}>💬</span>
            <span style={{ fontSize: '10px' }}>Чаты</span>
          </button>
          <button onClick={() => { setActiveTab('profile'); setIsSupportMode(false); }} style={{ flex: 1, padding: '10px 4px 8px', background: 'transparent', border: 'none', color: activeTab === 'profile' ? theme.primary : '#64748b', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
            <span style={{ fontSize: '20px', filter: activeTab === 'profile' ? `drop-shadow(0 0 6px ${theme.glow})` : 'none' }}>⚙️</span>
            <span style={{ fontSize: '10px' }}>Настройки</span>
          </button>
          {isDeveloper && (
            <button onClick={() => { setActiveTab('tickets'); setIsSupportMode(false); }} style={{ flex: 1, padding: '10px 4px 8px', background: 'transparent', border: 'none', color: activeTab === 'tickets' ? theme.primary : '#64748b', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
              <span style={{ fontSize: '20px', filter: activeTab === 'tickets' ? `drop-shadow(0 0 6px ${theme.glow})` : 'none' }}>🎫</span>
              <span style={{ fontSize: '10px' }}>Тикеты</span>
            </button>
          )}
          {isDeveloper && (
            <button onClick={() => { setActiveTab('users'); setIsSupportMode(false); fetchAllUsers(); }} style={{ flex: 1, padding: '10px 4px 8px', background: 'transparent', border: 'none', color: activeTab === 'users' ? theme.primary : '#64748b', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
              <span style={{ fontSize: '20px', filter: activeTab === 'users' ? `drop-shadow(0 0 6px ${theme.glow})` : 'none' }}>👥</span>
          ...
          <>
            ...
            {pinnedMsgData && (
              <div style={{ padding: '8px 16px', background: 'rgba(56, 189, 248, 0.1)', borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: theme.primary, minWidth: 0 }}>
                ...
                <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: '1 1 0%', minWidth: 0, color: '#fff' }}>
            ...
            {activeChatData?.type === 'channel' && activeChatData.owner_id !== session.user.id ? (
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                {showAttachMenu && (
                  <div style={{ position: 'absolute', bottom: '100%', left: '10px', marginBottom: '8px', background: '#0b0f19', border: `1px solid ${theme.border}`, borderRadius: '16px', padding: '8px', display: 'flex', flexDirection: 'column', gap: '4px', boxShadow: '0 4px 20px rgba(0,0,0,0.5)', zIndex: 20 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '10px', cursor: 'pointer', color: '#fff', whiteSpace: 'nowrap' }}>
                    ...
                    {(activeChatData?.type === 'group' || activeChatData?.type === 'channel') && (
                      <button type="button" onClick={() => { setShowAttachMenu(false); setShowPollModal(true); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '10px', background: 'transparent', border: 'none', cursor: 'pointer', color: isAura ? '#fff' : '#64748b', whiteSpace: 'nowrap', textAlign: 'left' }}>
                        📊 Опрос {!isAura && '(Аура)'}
                      </button>
                    )}
                  </div>
                ...
                <form onSubmit={e => { e.preventDefault(); sendMessage(); }} style={{ padding: '10px', background: '#0b0f19', display: 'flex', gap: '8px', alignItems: 'center', borderTop: `1px solid ${theme.border}` }}>
                  <button type="button" onClick={() => setShowAttachMenu(v => !v)} style={{ padding: '10px', background: showAttachMenu ? theme.primary : 'rgba(30, 41, 59, 0.8)', borderRadius: '50%', color: showAttachMenu ? '#000' : theme.primary, border: 'none', cursor: 'pointer', fontSize: '18px', lineHeight: 1, flexShrink: 0 }}>➕</button>
                  <button type="button" onClick={isRecording ? stopRecording : startRecording} style={{ padding: '10px', background: isRecording ? '#ef4444' : 'rgba(30, 41, 59, 0.8)', border: 'none', borderRadius: '50%', color: '#fff', cursor: 'pointer', flexShrink: 0 }}>{isRecording ? <Icons.Stop/> : <Icons.Mic/>}</button>
                  <input style={{ flex: 1, minWidth: 0, padding: '12px', borderRadius: '20px', background: '#030712', color: '#fff', border: `1px solid ${theme.border}`, outline: 'none' }} value={newMessage} onChange={e => handleTyping(e.target.value)} placeholder="Сообщение..." />
