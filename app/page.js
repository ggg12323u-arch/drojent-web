...
export default function Home() {
  ...
  const cleanupCall = () => {
    ...
    setCallState('idle'); setCallPeerInfo(null); setIsMuted(false);
  };
  ...
  const startCall = async (targetUser) => {
    ...
    try {
      ...
      setCallPeerInfo(targetUser); setCallState('outgoing');
      const pc = createPeerConnection(targetUser.id);
      stream.getTracks().forEach(t => pc.addTrack(t, stream));
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      sendSignal(targetUser.id, 'incoming-call', { callId: currentCallIdRef.current, offer, callerId: session.user.id, callerName: myProfile?.full_name || myProfile?.username, callerAvatar: myProfile?.avatar_url });
    } catch (e) { alert('Микрофон недоступен: ' + e.message); cleanupCall(); }
  };

  const acceptCall = async () => {
    if (!pendingOfferRef.current) return;
    const { callId, offer, callerId, callerName, callerAvatar } = pendingOfferRef.current;
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      localStreamRef.current = stream;
    ...
    } catch (e) { alert('Микрофон недоступен: ' + e.message); declineCall(); }
  };
  ...
  return (
    <div style={{ height: '100vh', width: '100vw', display: 'flex', background: '#030712', fontFamily: 'system-ui, sans-serif', color: '#f8fafc', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', background: 'rgba(11, 15, 25, 0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${theme.border}`, flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                {activeChatData?.type === 'dm' && activeUser && activeUser.id !== session.user.id && (
                  <button onClick={() => startCall(activeUser)} disabled={callState !== 'idle'} style={{ background: 'transparent', border: 'none', color: theme.primary, cursor: callState === 'idle' ? 'pointer' : 'not-allowed', fontSize: '20px' }}>📞</button>
                )}
      ...
      {/* Оверлей звонка */}
      {callState !== 'idle' && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(3,7,18,0.97)', zIndex: 9000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff', gap: '16px' }}>
          <div style={{ width: '96px', height: '96px', borderRadius: '50%', overflow: 'hidden', background: theme.secondary, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', boxShadow: `0 0 30px ${theme.glow}` }}>
            {callPeerInfo?.avatar_url ? <img src={callPeerInfo.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="peer"/> : '👤'}
          </div>
          <h2 style={{ margin: 0 }}>{callPeerInfo?.full_name || callPeerInfo?.username || 'Абонент'}</h2>
          ...
          <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
            {callState === 'incoming' ? (
              <>
                ...
                <button onClick={acceptCall} style={{ width: '60px', height: '60px', borderRadius: '50%', background: '#22c55e', border: 'none', color: '#fff', fontSize: '24px', cursor: 'pointer' }}>📞</button>
              </>
            ) : (
              <>
                {callState === 'active' && (
                  <button onClick={toggleMute} style={{ width: '54px', height: '54px', borderRadius: '50%', background: isMuted ? theme.primary : 'rgba(255,255,255,0.15)', border: 'none', color: isMuted ? '#000' : '#fff', fontSize: '20px', cursor: 'pointer' }}>{isMuted ? '🔇' : '🎙️'}</button>
                )}
