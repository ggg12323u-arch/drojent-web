...
export default function Home() {
  ...
  // Без него звонок работает "через раз" — это ограничение чистого STUN, а не баг кода.
  // Задай NEXT_PUBLIC_TURN_URL / _USERNAME / _CREDENTIAL в Vercel (см. calls-setup.md), и он подключится сам.
  const ICE_SERVERS = [
  ...
  const VIDEO_CONSTRAINTS = { width: { ideal: 1920 }, height: { ideal: 1080 }, frameRate: { ideal: 30 } };
  ...
  const boostVideoQuality = async (pc) => {
    ...
    try {
      ...
      params.encodings[0].maxBitrate = 3_500_000; // ~3.5 Мбит/с — комфортно для 1080p
