...
export default function Home() {
  ...
  const createCommunity = async () => {
    ...
    if (!isDeveloper) {
      ...
      if (existing && existing.length >= limit) { alert(`Лимит: ${limit} ${communityType === 'channel' ? 'канал(а/ов)' : 'групп(а/ы)'} на аккаунт (сейчас у вас: ${existing.length}).${isAura ? '' : ' Оформите Аура для увеличения лимита.'}`); return; }
    }
          ...
          <div style={{ padding: '20px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ textAlign: 'center', marginBottom: '10px' }}>
              <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: theme.secondary, margin: '0 auto 10px', overflow: 'hidden', border: `2px solid ${theme.primary}`, boxShadow: `0 0 15px ${theme.glow}` }}>
                {avatarUrl ? <img src={avatarUrl} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="avatar"/> : <DefaultAvatar seed={session.user.id} label={editFullName || editUsername} size={80} />}
              </div>
            ...
            <div style={{ background: 'var(--input-bg)', padding: '12px', borderRadius: '12px', border: `1px solid ${theme.border}` }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={async () => { setColorMode('dark'); const { error } = await supabase.from('profiles').update({ color_mode: 'dark' }).eq('id', session.user.id); if (error) alert('Не сохранилось: ' + error.message + '\n\nВыполните последнюю миграцию SQL в Supabase.'); }} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: colorMode === 'dark' ? `2px solid ${theme.primary}` : `1px solid ${theme.border}`, background: '#0b0f19', color: '#f8fafc', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>🌙 Тёмная</button>
                <button onClick={async () => { setColorMode('light'); const { error } = await supabase.from('profiles').update({ color_mode: 'light' }).eq('id', session.user.id); if (error) alert('Не сохранилось: ' + error.message + '\n\nВыполните последнюю миграцию SQL в Supabase.'); }} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: colorMode === 'light' ? `2px solid ${theme.primary}` : `1px solid ${theme.border}`, background: '#f1f5f9', color: '#0f172a', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>☀️ Светлая</button>
              </div>
            </div>
